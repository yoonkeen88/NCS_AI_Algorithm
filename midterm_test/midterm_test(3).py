"""
3. Fashion MNIST 의류 이미지 분류 모델을 만들어 보시오
...
- 신경망 구조: Flatten → Dense(256) + ReLU → Dropout(0.3) → Dense(128) + ReLU → Dense(10) + Softmax
- 손실 함수 : mean_squared_error (MSE) - 옵티마이저 : SGD
- 출력층 이전 라벨 전처리 : one-hot encoding 필수
- Batch size: 64
- epochs = 5번만 실행
